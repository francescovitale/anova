...
def compute_SS(residuals, model, experimental_data, factor_names):
    ...
    if n_factors == 2: # two-factor full-factorial
        ...
        SS["A"] = factor_levels_cardinality[factor_names[0]] * n_repetitions * sum(squared_first_factor_effects)

        SS["B"] = factor_levels_cardinality[factor_names[1]] * n_repetitions * sum(squared_second_factor_effects)
...
def check_statistical_significance(SSs, dfs):
    ...
    test_statistic = (SS["A"]/df["A"])/(SSs["E"]/dfs["E"])

    print("The computed test statistic with the F distribution is: " + str(test_statistic))

    if test_statistic > F_value:
        return True
    else:
        return False
    '''
...
def check_statistical_significance_non_parametric(experimental_data, factor_names, conf_level):
    ...
    if n_factors == 2: # two-factor full-factorial
        ...
        test_statistic_second_factor, p_value_second_factor = stats.friedmanchisquare(*second_factor_groups)

        test_statistic_combined_factor, p_value_combined_factor = stats.friedmanchisquare(*combined_factor_groups)
...
def print_ANOVA_results(dataset_name, groups_mean, groups_variance, model, normality_check_result, omoscedasticity_check_result, SS, statistical_significance_result, p_values, conf_level, factor_names, min_p_value_idx, min_p_value):
...
for dataset in experimental_data:
    ...
    if normality_check_result == True and omoscedasticity_check_result == True:
        statistical_significance_result, p_values = check_statistical_significance(SS, df)
    else:
    ...
    print_ANOVA_results(dataset,groups_mean, groups_variance, model, normality_check_result, omoscedasticity_check_result, SS, statistical_significance_result, p_values, conf_level, factor_names, min_p_value_idx, min_p_value)
